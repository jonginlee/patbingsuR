...
doSimulationAllFeaturesV2 <- function(data, cut, idx, window_size, window_step, save_filename, plotting = FALSE, type = 1, delay=1, startMilli=2000, endMilli=2000, thresholdvar = 0.1)
{
  ...
  for(i in 1:window_num)
  {
    ...
    start_milli <-paste( getHMS( ( + 51*60*1000 + 29*1000  -1186 + window_data$time[1])/((1000)*60*60)), window_data$time[1] )
    end_milli <- paste( getHMS( (+ 51*60*1000 + 29*1000  -1186 + window_data$time[nrow(window_data)])/((1000)*60*60) ), window_data$time[nrow(window_data)] )
